...
const Layout = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<section class="flex justify-center flex-wrap-reverse pt-10"><aside class="max-w-xs mr-auto sm:mr-6 sm:max-w-[12rem] font-light space-y-4 text-slate-600"><h3 class="font-bold" data-svelte-h="svelte-g3vaeu">Related Posts</h3> <ul class="text-sm font-light space-y-3">${each([1, 2, 3, 4, 5, 6], (a) => {
    return `${each(data.posts, (post) => {
      return `<li class="hover:underline"><a href="${escape(base, true) + "/posts/" + escape(post.slug, true)}"><span class="text-emerald-600">${escape(post.title)}</span> <p class="font-extralight md:text-xs text-sm text-slate-500 forest:text-slate-400">${escape(post.description.substring(0, 100))}...</p> <span class="font-extralight md:text-xs text-sm text-slate-400">${escape(post.date)} </span></a> </li>`;
    })}`;
